...
const SAMPLE_REPOS = [
  ...
  {
    name: "ayussh-2/repo-assistant",
    url: "https://github.com/ayussh-2/repo-assistant",
    desc: "askRepo Intelligence Engine",
  },
];
...
export const IngestScreen: React.FC<IngestScreenProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-6 md:p-12 w-full max-w-3xl mx-auto">
      ...
      {/* Try Out Sample Repositories Section */}
      <div className="w-full max-w-md mt-6 space-y-2.5">
        <h3 className="text-[11px] text-[#8a8f98] tracking-widest flex items-center gap-1.5">
          <Sparkles size={13} className="text-[#5e6ad2]" /> Try Popular
          Repositories
        </h3>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {SAMPLE_REPOS.map((sample) => {
            const isIndexed = repos.some(
              (r) => r.repo_name.toLowerCase() === sample.name.toLowerCase(),
            );
            return (
              <button
                key={sample.name}
                ...
                onClick={() => {
                  if (isIndexed) {
                    onSelectRepo(sample.name);
                  } else {
                    onChangeUrl(sample.url);
                  }
                }}
                className="p-3 rounded-xl border border-[#23252a] bg-[#0d0d0e] hover:bg-[#131416] hover:border-[#5e6ad2]/40 transition-all text-left flex flex-col justify-between group cursor-pointer"
              >
                <div className="flex items-center justify-between w-full gap-2">
                  <span className="text-xs font-semibold text-[#f7f8f8] group-hover:text-[#5e6ad2] transition-colors truncate">
                    {sample.name}
                  </span>
                  {isIndexed ? (
                    <Badge
                    ...
                    <ArrowUpRight
                      size={13}
                      className="text-[#8a8f98] group-hover:text-[#f7f8f8] transition-colors shrink-0"
                    />
                  )}
                </div>
                <span className="text-[11px] text-[#62666d] mt-1 truncate">
                  {sample.desc}
                </span>
              </button>
            );
